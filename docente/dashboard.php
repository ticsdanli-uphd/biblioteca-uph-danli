...
$st=$conn->prepare("SELECT nombre,email,telefono FROM docentes WHERE usuario_id=? AND sede_id=? LIMIT 1");
$sid=DANLI_SEDE_ID; $st->bind_param('ii',$uid,$sid); $st->execute(); $perfil=$st->get_result()->fetch_assoc() ?: []; $st->close();
$st=$conn->prepare("SELECT COUNT(*) total FROM registro_visitas WHERE beneficiario_usuario_id=? AND tipo='prestamo' AND devuelto=0");
...
$st=$conn->prepare("SELECT COUNT(*) total FROM registro_visitas WHERE beneficiario_usuario_id=? AND tipo='prestamo' AND devuelto=0 AND fecha_devolucion_esperada<CURDATE()");
$st->bind_param('i',$uid); $st->execute(); $vencidos=(int)$st->get_result()->fetch_assoc()['total']; $st->close();
include '../includes/header.php';
?>
<div class="container-fluid py-4">
<div class="mb-4"><h2>Bienvenido(a), <?=htmlspecialchars($perfil['nombre'] ?? $_SESSION['nombre_completo'])?></h2><p class="text-muted">Portal de Biblioteca UPH · Docente · Danlí</p></div>
...
<div class="col-12 col-md-6"><a href="/biblioteca/docente/prestamos.php" class="text-decoration-none"><div class="card border-0 shadow-sm h-100"><div class="card-body"><h4><i class="fas fa-book-reader me-2"></i>Mis préstamos</h4><p class="text-muted mb-0">Consulta fechas de devolución y estado.</p></div></div></a></div>
</div></div>
